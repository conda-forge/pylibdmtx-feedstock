...
Once the `conda-forge` channel has been enabled, `pylibdmtx` can be installed with `conda`:
...
```
mamba install pylibdmtx
```

It is possible to list all of the versions of `pylibdmtx` available on your platform with `conda`:
...
```
mamba search pylibdmtx --channel conda-forge
```
...
mamba repoquery search pylibdmtx --channel conda-forge

# List packages depending on `pylibdmtx`:
mamba repoquery whoneeds pylibdmtx --channel conda-forge

# List dependencies of `pylibdmtx`:
mamba repoquery depends pylibdmtx --channel conda-forge
```
